...
def manager_node(state: AgentState) -> dict:
    """
    Thin pass-through node — logs the incoming action and returns
    an empty state update.  The actual routing is done by
    manager_router() which is wired as a conditional edge.
    """
...
def manager_router(state: AgentState):
    ...
    Used with: workflow.add_conditional_edges("manager", manager_router)

    Returns:
        list[Send] — dispatch to subscriber nodes with filtered state
        END        — workflow complete
    """
    messages = state.get("messages", [])
    task_id = state.get("task_id", "unknown")

    # ── Cold start — no messages yet ────────────────────────────────────
    if not messages:
        emit_event(task_id, {
            "type": "log",
            "message": "🧠 Router: Cold start → routing to spec_writer"
        })
    ...
    # ── Terminal actions → workflow done ─────────────────────────────────
    ...
    if subscribers:
        emit_event(task_id, {
            "type": "log",
            "message": f"🧠 Router: Registry hit → dispatching to {subscribers}"
        })
        return [
    ...
    # ── LLM fallback (flexible routing for unregistered actions) ────────
    emit_event(task_id, {
        "type": "log",
